<#
.SYNOPSIS
    Launch the Genie PLATFORM only — every core service EXCEPT the sample agents.

.DESCRIPTION
    Starts the platform's own processes so that YOUR agents (run separately) can
    self-register against it. The bundled demo agents (weather/outage/rag) are NOT
    started — the registry comes up empty until an agent registers.
    ...
    To run the whole stack WITH the sample agents, use scripts\run-full.ps1.
...
# Importable code lives under src/ (the `genie` + `applications` packages), while
# `services.*` and the `app` entry resolve from the repo root. Putting src/ on
# PYTHONPATH makes every process importable.
...
# We match the launch command line, which carries a unique `RawUI.WindowTitle='...'`
# marker that only our service windows have.
# --------------------------------------------------------------------------
...
# mlflow_backend_store_uri). Must come up before the app so spans land somewhere.
# --------------------------------------------------------------------------
...
# Core backend services (MCP tools, registry, RAG retrieval) — NO sample agents.
...
Write-Host ""
Write-Host "Platform launched WITHOUT sample agents (each service has its own window)." -ForegroundColor Green
Write-Host "  Chat UI  : http://127.0.0.1:8000"
Write-Host "  Trace UI : http://127.0.0.1:8000/trace.html"
Write-Host "  Registry : http://127.0.0.1:8002/agents  (empty until an agent registers)"
Write-Host "  MLflow   : $mlflowUri"
Write-Host ""
Write-Host "Start your own agents separately (set AGENT_PORT); they self-register."
Write-Host "For the bundled sample agents, use scripts\run-full.ps1 instead."
