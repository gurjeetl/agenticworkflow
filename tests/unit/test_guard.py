...
def test_scanner_error_fails_closed() -> None:
    ...
    Patches a real scanner's ``scan`` so the failure originates inside the scan
    loop — exercising fail-closed for both the parallel and sequential paths
    (the parallel path no longer routes through ``llm_guard.scan_prompt``).
    """
    guard = get_llm_guard()
    target = guard._input_scanners[0]  # PromptInjection — a blocking classifier
    original = target.scan

    def boom(*_a, **_k):
        raise RuntimeError("simulated scanner failure")

    target.scan = boom  # type: ignore[method-assign]
    try:
        res = guard.scan_input("anything")
    finally:
        target.scan = original  # type: ignore[method-assign]
    assert res["valid"] is False
